...
- The showcase (`src/core/showcase.js`) builds its floor plan, pavilion index, cards and every count from `DEMOS` and `PAVILIONS`; a game appears under the pavilion named by its `pavilionId`. Pavilion names must keep the `"<Roman numeral>. <Title> (<Gloss>)"` shape and game names `"<Title> (<Subtitle>)"`, since both are split for display.
- Showcase thumbnails load from `assets/thumbs/<key>.webp`, fall back to the numbered `assets/screenshots/NN_<key>.png`, then to a hatched "Drawing pending" plate. After adding a game, run `node scripts/capture-thumbs.js` with the server up; it captures only games missing a thumb (`--all` redoes every one, or pass keys). It crops the engine canvas, so no app chrome appears in thumbs.
- Catalog order still determines the numbered fallback screenshot paths. Adding/reordering engines also affects hardcoded browser QA keys/counts and the separate `CODEX_DATA` registry in `src/core/codex.js`.
