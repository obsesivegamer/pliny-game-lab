...
- 2026-09-20T20:30:00-04:00 [CODE] Extended mobile/high-DPI engine contract across all 49 remaining engines in the showcase (all 50 engines now compliant):
  1. Added `uiScale() { return Math.max(1, this.dpr || 1); }` to every engine.
  2. Added `worldView() { return { x: 0, y: 0, w: this.width, h: this.height }; }` (or domain-specific aspect-preserving views) to prevent aspect distortion on non-desktop viewports.
  3. Wired `attachTouchBridge(this, canvas)` in constructors and `detachTouchBridge(this, this.canvas)` in `destroy()`.
  4. Wrapped HUDs in `ctx.scale(ui, ui)` with `sw = this.width / ui; sh = this.height / ui; narrow = sw < 560;` to render crisp, legible text at native CSS pixel scales on high-DPI screens and compact layouts on mobile viewports (e.g. 375px/390px).
  5. Verified all 50 engines clean: `node tests/verify-engines.js` (50 PASSED), `node tests/visual-qa.js` (50 CLEAN), `./scripts/verify-all.sh` (PASSED), `node tests/mobile-layout-qa.js` (PASSED), `node tests/showcase-qa.js` (PASSED), `node tests/codex-qa.js` (PASSED), `node tests/audio-qa.js` (PASSED), and `node tests/vesuvius-browser-smoke.js` (PASSED).
...
- 2026-09-20T20:30:00-04:00 [CODE] In canvas simulations with embedded pointer hit testing (e.g. `signal_fire` Polybius grid, `hoplite_phalanx` hoplite inspector), hitboxes computed from world/backing-store pixels must remain unscaled by `uiScale()` while UI HUD chrome scales cleanly, avoiding coordinate mismatch.
...
- All 50 engines implement `uiScale()`, `worldView()`, touch bridge attachment/detachment, and responsive mobile HUDs.
